...
class Cnn10_512(nn.Module):
    def __init__(self):
        super(Cnn10_512, self).__init__()
        self.stft = STFT()
        ...
        # 512차원 출력까지만 사용 (527차원 변환 레이어 제거)
        self.fc1 = nn.Linear(512, 512, bias=True)
        # self.fc_audioset = nn.Linear(512, 527, bias=True)  <-- 제거됨!
    ...
    def forward(self, audio):
        ...
        # 여기서 512차원의 순수 특징이 나옵니다.
        x = F.relu(self.fc1(x))
        
        # 527차원 변환 없이 바로 정규화해서 출력합니다.
...
def build_512():
    ...
    output_path = "models_official/smartwave_cnn10_e2e_512.onnx"
    print("Exporting ONNX to 512-dim...")
    audio = torch.randn(1, 160000)
    torch.onnx.export(
        model, audio, output_path,
